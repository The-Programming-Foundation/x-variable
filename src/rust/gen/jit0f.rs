    ...
    0x12 | 0x112 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F12_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F12_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
                *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
            ...
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F12_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F12_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x13 | 0x113 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F13_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F13_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
                *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F13_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F13_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
                *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
    ...
    0x14 | 0x114 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F14_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F14_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F14_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F14_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x15 | 0x115 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F15_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F15_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F15_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F15_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x2A | 0x12A => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F2A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F2A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F2A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F2A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F2A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F2A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F2A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F2A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x2E | 0x12E => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F2E_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F2E_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F2E_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F2E_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x2F | 0x12F => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F2F_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F2F_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F2F_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F2F_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x51 | 0x151 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F51_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F51_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F51_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F51_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F51_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F51_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F51_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F51_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x52 | 0x152 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_F3 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F52_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F52_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F52_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F52_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x53 | 0x153 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_F3 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F53_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F53_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F53_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F53_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x54 | 0x154 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F54_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F54_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F54_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F54_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x55 | 0x155 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F55_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F55_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F55_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F55_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x56 | 0x156 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F56_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F56_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F56_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F56_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x57 | 0x157 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F57_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F57_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F57_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F57_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x58 | 0x158 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F58_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F58_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F58_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F58_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F58_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F58_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F58_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F58_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x59 | 0x159 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F59_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F59_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F59_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F59_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F59_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F59_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F59_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F59_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x5A | 0x15A => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F5A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F5A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F5A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F5A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F5A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F5A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F5A_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F5A_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x5B | 0x15B => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F5B_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F5B_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F5B_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F5B_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F5B_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F5B_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x5C | 0x15C => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F5C_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F5C_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F5C_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F5C_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F5C_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F5C_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F5C_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F5C_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x5D | 0x15D => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F5D_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F5D_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F5D_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F5D_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F5D_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F5D_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F5D_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F5D_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x5E | 0x15E => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F5E_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F5E_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F5E_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F5E_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F5E_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F5E_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F5E_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F5E_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x5F | 0x15F => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F5F_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F5F_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F20F5F_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F20F5F_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_F30F5F_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_F30F5F_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F5F_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F5F_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x74 | 0x174 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F74_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F74_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x75 | 0x175 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F75_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F75_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F75_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F75_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0x76 | 0x176 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_660F76_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_660F76_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                let addr = ::modrm::decode(ctx.cpu, modrm_byte);
                ::codegen::gen_modrm_resolve(ctx, addr);
                ::codegen::gen_modrm_fn1(ctx.builder, "instr_0F76_mem", (modrm_byte >> 3 & 7) as u32);
            }
            else {
                ::codegen::gen_fn2_const(ctx.builder, "instr_0F76_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
    ...
    0xC2 | 0x1C2 => {
        ...
        if ctx.cpu.prefixes & ::prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                ...
                ::codegen::gen_modrm_resolve(ctx, addr);
                let imm = ctx.cpu.read_imm8() as u32;
                ::codegen::gen_modrm_fn2(ctx.builder, "instr_F20FC2_mem", (modrm_byte >> 3 & 7) as u32, imm);
            }
            else {
                let imm = ctx.cpu.read_imm8() as u32;
                ::codegen::gen_fn3_const(ctx.builder, "instr_F20FC2_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32, imm);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
            ...
            if modrm_byte < 0xC0 {
                ...
                ::codegen::gen_modrm_resolve(ctx, addr);
                let imm = ctx.cpu.read_imm8() as u32;
                ::codegen::gen_modrm_fn2(ctx.builder, "instr_F30FC2_mem", (modrm_byte >> 3 & 7) as u32, imm);
            }
            else {
                let imm = ctx.cpu.read_imm8() as u32;
                ::codegen::gen_fn3_const(ctx.builder, "instr_F30FC2_reg", (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32, imm);
            }
            *instr_flags |= ::jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            ::codegen::gen_move_registers_from_memory_to_locals(ctx);
        }
